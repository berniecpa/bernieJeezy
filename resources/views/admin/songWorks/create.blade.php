...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.songWork.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.song-works.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('song_title') ? 'is-invalid' : '' }}" name="song_title_id" id="song_title_id">
                    @foreach($song_titles as $id => $song_title)
                        <option value="{{ $id }}" {{ old('song_title_id') == $id ? 'selected' : '' }}>{{ $song_title }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('keyword') ? 'is-invalid' : '' }}" type="text" name="keyword" id="keyword" value="{{ old('keyword', '') }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('rhyming_keywords') ? 'is-invalid' : '' }}" name="rhyming_keywords" id="rhyming_keywords">{{ old('rhyming_keywords') }}</textarea>
