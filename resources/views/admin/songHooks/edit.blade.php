...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.songHook.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.song-hooks.update", [$songHook->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('title') ? 'is-invalid' : '' }}" name="title_id" id="title_id">
                    @foreach($titles as $id => $title)
                        <option value="{{ $id }}" {{ (old('title_id') ? old('title_id') : $songHook->title->id ?? '') == $id ? 'selected' : '' }}>{{ $title }}</option>
                    @endforeach
                </select>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('song_hook') ? 'is-invalid' : '' }}" name="song_hook" id="song_hook">{{ old('song_hook', $songHook->song_hook) }}</textarea>
